...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Guest Profiles" value={kpiVal('Guest Profiles', '4.2M')} status="neutral" />
        <KPICard title="Repeat Rate" value={kpiVal('Repeat Rate', '34%')} status="neutral" />
        <KPICard title="NPS Score" value={kpiVal('NPS Score', '72')} status="neutral" />
        <KPICard title="Active Campaigns" value={kpiVal('Active Campaigns', '18')} status="neutral" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'NPS' }]}
          title="Guest Satisfaction (Monthly NPS)"
        />
        <Chart
          ...
          type="bar"
          xKey="category"
          yKeys={[{ key: 'count', name: '฿K' }]}
          title="Revenue per Guest by Segment"
        />
      </div>
        </div>
      </div>
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Offer Conversion" value={kpiVal('Offer Conversion', '12.4%')} />
        <KPICard title="Upsell Revenue" value={kpiVal('Upsell Revenue', '฿84M')} />
        <KPICard title="App Engagement" value={kpiVal('App Engagement', '67%')} />
      </div>
      <Chart
        data={data?.detail || [{ x: 'Mon', y: 24 }, { x: 'Tue', y: 28 }, { x: 'Wed', y: 22 }, { x: 'Thu', y: 31 }, { x: 'Fri', y: 26 }, { x: 'Sat', y: 19 }, { x: 'Sun', y: 23 }]}
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Conversion %' }]}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'Churn Risk %' }]}
          title="Churn Probability by Tier"
        />
        <ActionMemo
          persona={{ name: 'Siriporn Vacharapan', role: 'CRM Director' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Launch win-back for 12K at-risk Platinum guests', 'A/B test wellness package for Japanese segment', 'Personalize anniversary offers for couples'],
              },
              {
    ...
    <div className="h-[600px]">
      <AskAI
        ...
        sampleQuestions={[
          ...
          'Show churn prediction for Platinum tier',
          'What offers convert best for Chinese visitors?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Personalization', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Loyalty & Retention', icon: '⚡', content: domainTab2 },
